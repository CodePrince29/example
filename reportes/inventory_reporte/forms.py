...
from warehouses.warehouse_entrance.models import WarehouseEntranceConfirmation
...
class AdvancedFilterFormSet(BaseFormSet):
    ...
    def __init__(self, *args, **kwargs):       
        ...
        [inventory_fields.append(("{}".format(field.name), field.verbose_name)) for field in WarehouseInventory._meta.get_fields() if not field.is_relation]
        [customer_fields.append(("client__{}".format(field.name), field.verbose_name)) for field in Client._meta.get_fields() if not field.is_relation]
        [confirmation_fields.append(("warehouse_entrance_confirmation__{}".format(field.name), field.verbose_name)) for field in WarehouseEntranceConfirmation._meta.get_fields() if not field.is_relation]
        [product_fields.append(("product__{}".format(field.name), field.verbose_name)) for field in Product._meta.get_fields() if not field.is_relation]

        self.model_fields = tuple([
           (_('Warehouse Inventory'),tuple(inventory_fields)),
           (_('Customer'), tuple(customer_fields)),
           (_('Warehouse Product Entrance Confirmation'), tuple(confirmation_fields)),
           (_('Product'), tuple(product_fields)),
           ])
