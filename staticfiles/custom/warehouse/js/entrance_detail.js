...
function sum_table(){
  ...
  $("#entrance_boxes").val(parseInt(result_boxes))
...
$("#product_measurement_select_id").change(function(){
  ...
  boxes = $(this).find(':selected').data('control_total_boxes')
  ...
  $(".boxes").val(boxes)
  already_selected_measurement = $("#id_selected_product").val()
  ...
  jQuery.ajax( {
      ...
      //   alert("Información Incompleta del producto:" + product_code)
      // }
      selected_pallet = $("#id_selected_pallet").val()
      if(selected_pallet != "" && parseInt(measurement_id) == parseInt(already_selected_measurement)){
        $(".pallete_form_list").find(".table-row").addClass("hide")
        current_pallet = $('*[data-pallet-id=' + selected_pallet +']').closest(".table-row")
        current_pallet.removeClass("hide")
        all_previous = current_pallet.prevAll( '.table-row' ).length
        $(".current_pallet").val(all_previous + 1)
      }
      else{
...
$(document).on("click",".romaneo_submit", function () {
  ...
  peso_variable_data = []
  zero_peso_value = false

  total_peso_variable.each(function(){
    if(parseFloat($(this).val()) == 0){
      zero_peso_value = true
    }
  ...
  $.ajax({
      ...
      { console.log(response)
          ...
          if(response.code == 200){
            ...
            $(".pallete_form_list").find(".table-row:visible").find(".pallet_peso_variable_exists").val("True")
            if (zero_peso_value){
              $(".pallete_form_list").find(".table-row:visible").find(".pallet_peso_variable_with_zero_value").val("True")
            }
            else{
              $(".pallete_form_list").find(".table-row:visible").find(".pallet_peso_variable_with_zero_value").val("False")
            }
          }else{
...
// new_messurment_login_button

$(document).on('click', ".new_pallet", function() {
  $(".product_error_code").addClass("hide")
  $(".new_measurement_creation").modal("show")
  $("#id_menuobras_username").removeAttr("readonly")
  $("#id_menuobras_password").removeAttr("readonly")
  // new_messurment_login_button
  $(".new_messurment_login_button").removeClass("hide")
  $(".menuobras_login_button").addClass("hide")
...
$(document).on('blur', ".new_total_boxes", function() {
  ...
  prev_val = parseFloat($(".new_total_kg").val())
  if(prev_val > 0){

  }
  else{
    if(total_kg >0){
      $(".new_total_kg").val(total_kg.toFixed(3))
    }
  }
...
function addNewPallet(selector, position_i) {
  ...
  var position1  = $("#entrance_product_detail").find('.warehouseentranceconfirmation_set').length
  selector.find('.pallete_form_list').append(selector.find('.empty_pallate_detail_form').html().replace(/__prefix__/g, count).replace(/__prefixe__/g, count).replace(/__position__/g, position_i).replace(/place_position/g, parseInt(count)+1));
  var current_elm = $('.pallete_form_list').find(".table-row:visible")
  current_elm.find(".boxes").val(0)
  current_elm.find(".box_kg").val(0)
...
function button_hide_show_with_for_manioveras() {
  ...
  maniobra_value = maniobras.find("input.maniobras[type=text]").val()
